...
router.get("/status", (_req, res) => res.json(homeAssistantService.status()));
router.post("/test-connection", asyncHandler(async (req, res) => res.json(await homeAssistantService.testConnection(req.user!.id))));
router.get("/entities", asyncHandler(async (_req, res) => res.json(await homeAssistantService.entities())));
...
router.post("/conversation", validate(z.object({ text: z.string().min(1) })), asyncHandler(async (req, res) => res.json(await homeAssistantService.conversation(req.body.text))));
