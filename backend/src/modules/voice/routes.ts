...
router.post("/process", validate(z.object({ text: z.string().optional().default("") })), asyncHandler(async (req, res) => {
  ...
  const result = await brainService.ask({ userId: req.user!.id, message: text, source: "voice", mode: "normal", allowExternalAI: true, allowTools: true });
  res.json({ ...result, reply: applyJarvisVoicePersona(result.reply), answer: applyJarvisVoicePersona(result.answer), voicePersona: jarvisVoicePersona.name });
}));
